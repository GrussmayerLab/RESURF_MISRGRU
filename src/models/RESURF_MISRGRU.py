...
class EncoderUnit(nn.Module):
    def __init__(self, in_channels, num_res_layers, kernel_size, channel_size):
        ...
        res_layers = [
            ResidualBlock(channel_size, kernel_size) for _ in range(num_res_layers)
        ]
...
class ConvGRUUnit(nn.Module):
    ...
    def forward(self, x, prev_state):
        ...
        if prev_state is None:
            ...
            prev_state = torch.zeros(state_size, device=x.device)
        ...
        candidate_state = torch.tanh(
            self.out_gate(torch.cat([x, prev_state * reset], dim=1))
        )
...
class FusionModule(nn.Module):
    ...
    def forward(self, x, h=None):
        ...
        fused_representations = torch.sum(cur_layer_input, dim=1)
...
class Decoder(nn.Module):
    def __init__(self, dec_config):
        ...
        # Add the first deconvolution layer
        deconv_layers.append(
            nn.ConvTranspose2d(
                ...
                padding=dec_config["deconv"]["padding"],
                output_padding=dec_config["deconv"]["output_padding"],
            )
        )
        deconv_layers.append(nn.PReLU())

        # Add the hidden layers if num_hidden_layers > 1
        for _ in range(0, dec_config["deconv"]["num_hidden_layers"]):
            deconv_layers.append(
                nn.Conv2d(
